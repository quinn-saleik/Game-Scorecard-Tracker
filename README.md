...
- **Achievement badges** (23 total, shown on each player's page, locked/unlocked): career milestones (first win, win streaks, games played, games tried) plus game-specific ones built from real round-by-round data — Went Alone / Lone Wolf / Solo Streak (Pick Your Partner), Perfect Game / Zero Hero / Nil Streak (10+ hit zero-bids in one game, Oh Heck!), Untouchable / Survivor / Double Trouble (same-round set) / Comeback Kid (3-player Euchre), Shutout (Traditional Euchre), Skunked (2-player Euchre), Big Flip (Flip7), Iron Man / Clean Sweep (games in a single day), Camera Ready (full profile personalization — color + avatar + photo).
- **Hall of Fame** (`/hall-of-fame`): all-time records across the whole roster — most games played, most wins, longest win streak ever, Euchre Royalty (combined wins across every Euchre variant), Flip7 High Score (best single round, not final total), Table Regular (most distinct opponents played with), Rivalry (the most-frequent opponent pair, with each one's head-to-head win tally), and Biggest Achiever (whoever has unlocked the most achievement badges).
...
- **`players`** collection — one doc per player (`{ name, firstName, lastName, active, color, avatar, photo, groupIds }`). Added from the Me page (or the Setup screen's inline "add a player") — first + last name required; exact-duplicate names are blocked. No seeded roster — the app starts with an empty player list. "Removing" a player just flips `active: false` so past game stats still resolve their name. Each player can be given a color, an emoji avatar, and/or a photo (photo takes over from the emoji wherever they're shown), plus any number of `groupIds` — see "Who am I, the Me page, and groups" below.
- **`groups`** collection — one doc per group (`{ name, createdAt }`, doc id slugified from the name). Created from the Me page; a one-time migration seeds the pre-groups roster into a "Worthington Family" group the first time the app loads with none yet (see below).
...
## Who am I, the Me page, and groups

- **Who am I** (top-bar pill, every screen): pick which player you are from the roster — stored only in `localStorage` on that device (`src/data/whoami.js`), never synced or enforced server-side. Purely a convenience default: pre-selects you when starting a new game (still fully adjustable), floats your own row to the top of Stats, and drives the group filtering below. Works fine with nothing picked — a shared/passed-around phone, or someone who never bothers, sees the app exactly as before this feature existed.
- **Me page** (`/me`, nav icon 👤) replaced the old Players nav tab: your own stats (same breakdown as a player's detail page), your name/color/avatar/photo, and your group memberships, all in one place. If you haven't picked who you are yet, it offers the roster to choose from, or an "add yourself" form that creates a new player and picks you in one step — this is also now the only place a brand-new player normally gets created for themselves (starting a game still lets you add someone new inline too).
- **Groups** — a player can belong to any number of groups (`groupIds` on the player doc, managed from the Me page: tap to join/leave, or create a new one). The point is "cleaner UI for a big shared roster," not a permission system — there's still no login, and nothing here is enforced server-side. Once you've picked who you are and joined at least one group, every game's Setup screen, Stats, and Hall of Fame default to showing only players/games that share a group with you; someone in only one group sees only that group's people, while someone in several (Quinn: "Worthington Family" + "Nashville Friends") sees the union. No whoami picked, or your player isn't in any group: nothing is filtered, same as before groups existed. A lightweight admin view at `/players` (linked from the bottom of Me) still shows and edits everyone, including group membership, for the rare case of fixing someone else's info from a shared device.
