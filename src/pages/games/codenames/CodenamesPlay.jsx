...
export default function CodenamesPlay() {
  ...
  const threshold = session.config?.winThreshold || 1;
  ...
  async function deleteRound(index) {
    ...
    try {
      ...
      const newTotals = recomputeTotals("codenames", session, newRounds);
  ...
  return (
    <div>
      <h1 className="page-title" style={{ justifyContent: "space-between" }}>
        <span><span className="suit black">🕵️</span> Codenames</span>
        <TvMode gameName="Codenames" icon="🕵️" statusLine={`First to ${threshold}`} rows={tvRows} />
      </h1>
      ...
      {pendingFinish ? (
        <div className="card-surface">
          ...
          <p>Double-check the last game before locking it in.</p>
        ...
        <div className="card-surface">
          <h2>Who finished their board first?</h2>
          <div className="btn-row">
