...
export default function EgyptianRatscrewPlay() {
  ...
  const threshold = session.config?.winThreshold || 5;
  ...
  async function deleteRound(index) {
    ...
    try {
      ...
      const newTotals = recomputeTotals("egyptian-ratscrew", session, newRounds);
  ...
  return (
    <div>
      <h1 className="page-title" style={{ justifyContent: "space-between" }}>
        <span><span className="suit red">✋</span> Egyptian Ratscrew</span>
        <TvMode gameName="Egyptian Ratscrew" icon="✋" statusLine={`Hand ${rounds.length + 1} · first to ${threshold}`} rows={tvRows} />
      </h1>
