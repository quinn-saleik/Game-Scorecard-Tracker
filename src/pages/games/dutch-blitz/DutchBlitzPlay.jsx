...
export default function DutchBlitzPlay() {
  ...
  const threshold = session.config?.winThreshold || 75;
  ...
  async function deleteRound(index) {
    ...
    try {
      ...
      const newTotals = recomputeTotals("dutch-blitz", session, newRounds);
  ...
  return (
    <div>
      <h1 className="page-title" style={{ justifyContent: "space-between" }}>
        <span><span className="suit red">🔺</span> Dutch Blitz — Hand {rounds.length + 1}</span>
        <TvMode gameName="Dutch Blitz" icon="🔺" statusLine={`Hand ${rounds.length + 1} · first to ${threshold}`} rows={tvRows} />
      </h1>
