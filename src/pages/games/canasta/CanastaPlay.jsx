...
export default function CanastaPlay() {
  ...
  const targetScore = session.config?.targetScore || 5000;
  ...
  async function deleteRound(index) {
    ...
    try {
      ...
      const newTotals = recomputeTotals("canasta", session, newRounds);
  ...
  return (
    <div>
      <h1 className="page-title">🃞 Canasta — Hand {rounds.length + 1}</h1>
        ...
        <div className="card-surface">
          ...
          <form onSubmit={submitRound}>
            <div className="field">
              ...
              <ScorePresets values={[50, 100, 300, 500]} onPick={(v) => setInputA(String(v))} />
            </div>
            <div className="field">
              ...
              <ScorePresets values={[50, 100, 300, 500]} onPick={(v) => setInputB(String(v))} />
            </div>
            <div className="btn-row">
              {rounds.length > 0 && (
