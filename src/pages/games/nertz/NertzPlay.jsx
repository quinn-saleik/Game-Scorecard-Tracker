...
export default function NertzPlay() {
  ...
  const threshold = session.config?.winThreshold || 100;
  ...
  async function deleteRound(index) {
    ...
    try {
      ...
      const newTotals = recomputeTotals("nertz", session, newRounds);
  ...
  return (
    <div>
      <h1 className="page-title" style={{ justifyContent: "space-between" }}>
        <span><span className="suit red">⚡</span> Nertz — Hand {rounds.length + 1}</span>
        <TvMode gameName="Nertz" icon="⚡" statusLine={`Hand ${rounds.length + 1} · first to ${threshold}`} rows={tvRows} />
      </h1>
