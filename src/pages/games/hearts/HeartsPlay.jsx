...
export default function HeartsPlay() {
  ...
  const pendingFinish = Object.values(totals).some((t) => (t || 0) >= targetScore);
  const potentialWinners = session.players.filter(
    (p) => (totals[p.id] || 0) === lowestTotal
  );
  ...
  async function deleteRound(index) {
    ...
    try {
      ...
      const newTotals = recomputeTotals("hearts", session, newRounds);
  ...
  return (
    <div>
      ...
      <div className="card-surface">
        <h2>Scores (lowest wins, ends at {targetScore})</h2>
          ...
          <tbody>
            {standings.map((p) => (
              <tr key={p.id}>
                ...
                <td className={(totals[p.id] || 0) === lowestTotal ? "leader" : ""}>
                  {totals[p.id] || 0}
                </td>
              </tr>
            ))}
